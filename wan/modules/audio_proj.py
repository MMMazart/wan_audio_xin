...
class AudioProjModel(ModelMixin, ConfigMixin):
    ...
    def compress_audio(self, audio_feat):
        """
        zero-pad补齐, 保证与VAE压缩对齐
        """
        audio_feat = torch.cat([torch.zeros_like(audio_feat[:, :1].repeat(1, 3, 1, 1)), audio_feat], dim=1) # [bs, f+3, 32, 1536]
        bz, audio_len, blocks, channels = audio_feat.shape
        audio_feat = audio_feat.view(bz, audio_len//4, -1, channels)    # [bs, (f+3)/4, 32*4, 1536]
        return audio_feat
    ...
    def forward(self, audio_embeds):
        ...
        context_tokens = self.compress_audio(context_tokens)
